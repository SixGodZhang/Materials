...
![分配内存](2019_01_05_managed_heap_and_gc_principle\2019_01_05_newobj.png)
...
![分配内存](2019_01_05_managed_heap_and_gc_principle\2019_01_05_fs.png)
...
```
![分配内存](2019_01_05_managed_heap_and_gc_principle\2019_01_05_testa.png)
...
![](2019_01_05_managed_heap_and_gc_principle\2019_01_05_gc.png)
...
![](2019_01_05_managed_heap_and_gc_principle\2019_01_19_gc_notification.png)
...
![](2019_01_05_managed_heap_and_gc_principle\2019_01_12_object_finalize.png)  
如上，这是System.Object的源码，并没有直接发现Finalize方法，这或许是微软在编译mscolib.dll是处理这个的，有以下两种方式可以验证:  
__Ａ．通过dnspy反编译mscorlib.dll__  
![](2019_01_05_managed_heap_and_gc_principle\2019_01_12_object_finalize_1.png)

如上，反解出来可以发现~Object方法没有了，取而代之的是Finalize方法  

__Ｂ．查看Dll的元数据表__  
![](2019_01_05_managed_heap_and_gc_principle\2019_01_12_object_finalize_2.png)  
...
```
IL指令如下:  
![](2019_01_05_managed_heap_and_gc_principle\2019_01_12_object_finalize_3.png)  

如上，我们发现 __类__ 类型若写了终结器，会去调用基类System.Object的终结器

### CLR是如何处理终结器的(内部工作原理)?
Ａ．如果一个类型显示定义了Finalize方法，当调用该类型的实例构造器之前，会将指向该对象的指针放到一个终结列表中  
![](2019_01_05_managed_heap_and_gc_principle\2019_01_12_object_finalize_principle_1.png)  
Ｂ．当第一次GC发生时，当一个对象被标记为垃圾之后并且不再终结列表中，则会被直接回收.反之，则会在终结列表中删除该指针，并把它附加到foreachable队列中  
![](2019_01_05_managed_heap_and_gc_principle\2019_01_12_object_finalize_principle_2.png)  
![](2019_01_05_managed_heap_and_gc_principle\2019_01_12_object_finalize_principle_3.png)  
Ｃ．CLR会创建一个特殊的高优先级的线程将foreachable队列中的元素依次移除，并调用其Finalize方法.最后，在下一次GC的时候，已终结的对象才会被GC真正的回收  
![](2019_01_05_managed_heap_and_gc_principle\2019_01_12_object_finalize_principle_4.png)  
